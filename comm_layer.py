...
class Comm_layer:
    def __init__(self, start,dataset:Dataset):
        self.start = start
    ...
    def command(self, cmd):
        ...
        if "set_point" in cmd.keys():
            ...
            writeConfig = True
        if "k_p" in cmd.keys():
            self.dataset.update('k_p',float(cmd['k_p']))
            writeConfig = True
        if "k_i" in cmd.keys():
            self.dataset.update('k_i',float(cmd['ki']))
        ...
        if "setI" in cmd.keys():
            self.start.pid1.setI_err(
                float(cmd["setI"])/self.start.pid1.getK_i())
        ...
        if writeConfig:
            self.start.getCtlConfig()["SetPoint"] = str(
                self.start.getSetPoint())
